...
def main():
    ...
    potential_value_columns = ['amount', 'value', 'transaction_amount', 'total', 'price', 
                               'ledger_id', 'balance', 'transaction_value', 'sum', 'quantity']
    ...
    # First, try to find a column with a name suggesting it's a value/amount column
    for col in df.columns:
        col_lower = col.lower()
        for potential in potential_value_columns:
    ...
    # If no specific value column found, use the first numeric column (excluding ID-like columns)
    if value_column is None and numeric_columns:
        ...
        for col in numeric_columns:
            ...
            if 'id' not in col_lower:
                value_column = col
                break
        # If all numeric columns have 'id' in name, use the first one anyway
        if value_column is None:
            value_column = numeric_columns[0]
    
    if value_column:
        print(f"Using column '{value_column}' to determine High_Value status")
        ...
        print(f"Transactions marked as 'Yes' (above 100000): {high_value_count}")
        print(f"Transactions marked as 'No' (100000 or below): {low_value_count}")
    else:
        print("WARNING: No numeric column found to evaluate transaction values.")
